...
#include "SIAlembicCurve.h"
#include "SIAlembicXForm.h"

SIAlembicOCurves::SIAlembicOCurves(const XSI::CRef & in_ref, SIAlembicWriteJob * in_job)
...
void SIAlembicICurves::Check()
{
	AlembicICurves* c = (AlembicICurves*)this;
	if(!Alembic::AbcGeom::ICurves::matches((*c->GetIObject()).getMetaData()))
	{
		Clean();
	}
}

ABCStatus SIAlembicOCurves::Save(double time)
{
	Application app;
...
CStatus SIAlembicICurves::Update(int iFrame)
{
	Application().LogMessage(L"Alembic Update Curve...");
	AlembicICurves* c = (AlembicICurves*)this;
	Alembic::AbcGeom::ICurves curves((*c->GetIObject()),Alembic::Abc::kWrapExisting);	
