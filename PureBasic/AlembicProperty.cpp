...
{
	infos->_time = time;
	infos->_traits = GetDataTraits();
	infos->_type = GetPropertyType();
	infos->_nbitems = GetNbItems((Abc::chrono_t)time);
	return true;
...
{	
	...
	switch(_type){
		case PropertyType_Scalar:
		{
			...
			memcpy(sample->_datas, out, _scalar.getDataType().getNumBytes());
			break;
		}
		case PropertyType_Array:
		{
			Alembic::AbcCoreAbstract::ArraySamplePtr out;
			_array.get(out,selector);
			memcpy(sample->_datas, (void*)out->getData(), infos->_nbitems * _array.getDataType().getNumBytes());
			break;
		}
